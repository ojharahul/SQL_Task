...
class SqliteOperation:
    ...
    def Create_table(self, tabl, tabl_struct):
        ...
        try:
            if type(tabl_struct) == dict:
                ...
                return "Table is not correct"
        except Exception as e:
            self.log.exception(str(e))
            print(e)

    def select_data_table(self, tab1):
        """
        Select data form table
        """
        try:
            query = "select * from {}".format(tab1)
            self.cursor.execute(query)
            self.log.info(f"Executed Query: {query}")
            res = self.cursor.fetchall()
            self.log.warning(f"Returning data form table: {tab1}")
            return res
        except Exception as e:
            self.log.exception(str(e))
            print(e)
    ...
    def close_connection(self):
        ...
        try:
            self.db.close()
            self.log.info("Db connection is closed ")
        except Exception as e:
            self.log.exception(str(e))
            print(e)
